...
module.exports = {
    ...
    run: async (guild) => {
        ...
        let joinChannel = db.get(`join_channel_id_${guild.guild.id}`) || "null"
        let joinText = db.get(`join_text_id_${guild.guild.id}`) || "null"
            ...
            const channnelsend = guild.client.channels.cache.get(joinChannel)
                ...
                {   
                    ...
                    const GoodEmbed = new MessageEmbed()
                        .setTitle('Nowa osoba')
                        .setColor('#002d96')
                        .setDescription('<@' + user + '> Wbił na serwer **' + servername + '** jesteś naszą **' + membercount + '** osobą na serwerze!')
